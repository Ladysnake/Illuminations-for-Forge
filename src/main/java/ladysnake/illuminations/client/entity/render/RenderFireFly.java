...
public class RenderFireFly<E extends FireFlyEntity> extends EntityRenderer<E> {
    ...
    public void doRender(E entity, double x, double y, double z, float entityYaw, float partialTicks)
    {
        ...
        if (!this.renderOutlines)
        {
            ...
            // we could have one for each U and V, but both are the same, so we slapped them
            // into one min and one max
            float min = 0;
            float max = 1;
    ...
    /**
     * rounds a number to the' precision value' + 1 after the comma, rather then the
     * first value after the comma
     */
